...
import { ShoppingCart, Star, ArrowLeft, Shield, Check, X, Info } from "lucide-react"
...
const batmanMasks: BatmanMask[] = [
  {
    id: "bt-01",
    name: "Black Batman Mask",
    code: "BT-01",
    price: 500,
    image: "/images/batman/black-batman-mask.jpg",
    description: "Classic black Batman mask with pointed ears",
  },
  {
    id: "bt-02",
    name: "Batman Red Accents",
    code: "BT-02",
    price: 500,
    image: "/images/batman/batman-red-accents.jpg",
    description: "Batman mask with striking red eye and mouth accents",
  },
  {
    id: "bt-03",
    name: "Black Batwoman Mask",
    code: "BT-03",
    price: 500,
    image: "/images/batman/black-batwoman-mask.jpg",
    description: "Sleek Batwoman mask with elegant design",
  },
]
...
export default function BatmanPage() {
  ...
  const orderViaWhatsApp = (mask: BatmanMask) => {
  ...
  const orderSelectedMasks = () => {
    ...
    const selectedMasksList = batmanMasks.filter((mask) => selectedMasks.has(mask.id))
    ...
    const message = `Hi! I'd like to order the following Batman masks:\n\n${itemsList}\n\nTotal: KSH ${totalPrice}\n\nPlease confirm availability and delivery details.`
    const encodedMessage = encodeURIComponent(message)
    window.open(`https://wa.me/${whatsappNumber}?text=${encodedMessage}`, "_blank")
  }
  ...
  const totalPrice = batmanMasks.filter((mask) => selectedMasks.has(mask.id)).reduce((sum, mask) => sum + mask.price, 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-blue-900 relative overflow-hidden">
      ...
      <header className="relative z-10 p-4 md:p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center">
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-white bg-gradient-to-r from-gray-400 via-blue-500 to-black bg-clip-text text-transparent mb-2">
              BATMAN & BATWOMAN
            </h1>
            <p className="text-white/80 text-sm md:text-lg">Superhero Collection</p>
          </div>
        </div>
      </header>
      ...
      {selectedCount > 0 && (
        <div className="fixed top-24 left-0 right-0 z-40 px-4">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto">
            <div className="backdrop-blur-md bg-gradient-to-r from-blue-600/90 to-purple-600/90 rounded-2xl p-4 border border-white/20 shadow-xl">
          ...
          <div className="text-center mb-8">
            <div className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 inline-block">
              <div className="grid grid-cols-3 gap-6 text-center">
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-blue-400">{batmanMasks.length}</div>
                  <div className="text-white/70 text-xs md:text-sm">Hero Masks</div>
                </div>
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-gray-300">KSH 500</div>
                ...
                >
                  ...
                  >
                    <div className="relative">
                      <div className="aspect-square bg-gradient-to-br from-gray-500/20 to-blue-500/20 flex items-center justify-center relative overflow-hidden">
                        <img
      ...
      <footer className="relative z-10 py-12 px-4 border-t border-white/20 mt-20">
        <div className="max-w-4xl mx-auto text-center">
          <h3 className="text-2xl font-bold text-white mb-4">BATMAN & BATWOMAN COLLECTION</h3>
          <p className="text-white/70 mb-6">Superhero masks for heroic adventures</p>
          <div className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 inline-block">
