...
import { ShoppingCart, Star, ArrowLeft, Zap, Check, X, Info } from "lucide-react"
...
const ledMasks: LEDMask[] = [
  {
    id: "led-01",
    name: "LED Collection Set",
    code: "LED-01",
    price: 1200,
    image: "/images/led-masks/led-collection.jpg",
    description: "Complete LED mask collection in multiple colors",
    color: "Multi-Color",
  },
]

export default function LEDMasksPage() {
  ...
  const orderViaWhatsApp = (mask: LEDMask) => {
  ...
  const orderSelectedMasks = () => {
    ...
    const selectedMasksList = ledMasks.filter((mask) => selectedMasks.has(mask.id))
    ...
    const message = `Hi! I'd like to order the following LED masks:\n\n${itemsList}\n\nTotal: KSH ${totalPrice}\n\nPlease confirm availability and delivery details.`
    const encodedMessage = encodeURIComponent(message)
    window.open(`https://wa.me/${whatsappNumber}?text=${encodedMessage}`, "_blank")
  }
  ...
  const totalPrice = ledMasks.filter((mask) => selectedMasks.has(mask.id)).reduce((sum, mask) => sum + mask.price, 0)

  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-900 via-green-900 to-blue-900 relative overflow-hidden">
      ...
      {showGuide && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <Button
              className="w-full bg-gradient-to-r from-yellow-600 to-blue-600 hover:from-yellow-700 hover:to-blue-700 text-white font-medium py-3 rounded-xl shadow-lg"
              onClick={() => setShowGuide(false)}
            >
      ...
      <div className="absolute inset-0 opacity-20">
        ...
        <div className="absolute top-40 right-20 w-32 h-32 md:w-72 md:h-72 bg-green-500 rounded-full mix-blend-multiply filter blur-xl animate-pulse delay-1000"></div>
        <div className="absolute bottom-20 left-40 w-32 h-32 md:w-72 md:h-72 bg-blue-500 rounded-full mix-blend-multiply filter blur-xl animate-pulse delay-2000"></div>
      </div>
      ...
      <header className="relative z-10 p-4 md:p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center">
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-white bg-gradient-to-r from-yellow-400 via-green-500 to-blue-600 bg-clip-text text-transparent mb-2">
              LED MASKS
            </h1>
            <p className="text-white/80 text-sm md:text-lg">Futuristic Collection</p>
          </div>
        </div>
      </header>
      ...
      {selectedCount > 0 && (
        <div className="fixed top-16 left-0 right-0 z-40 px-4">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto">
            <div className="backdrop-blur-md bg-gradient-to-r from-yellow-600/90 to-blue-600/90 rounded-2xl p-4 border border-white/20 shadow-xl">
          ...
          <div className="text-center mb-8">
            <div className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 inline-block">
              <div className="grid grid-cols-3 gap-6 text-center">
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-yellow-400">{ledMasks.length}</div>
                  <div className="text-white/70 text-xs md:text-sm">LED Designs</div>
                </div>
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-green-400">KSH 1200</div>
                  <div className="text-white/70 text-xs md:text-sm">Each Mask</div>
                </div>
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-blue-400">Futuristic</div>
                  <div className="text-white/70 text-xs md:text-sm">Technology</div>
                </div>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  >
                    <div className="relative">
                      <div className="aspect-square bg-gradient-to-br from-yellow-500/20 to-green-500/20 flex items-center justify-center relative overflow-hidden">
                        <img
      ...
      <footer className="relative z-10 py-12 px-4 border-t border-white/20 mt-20">
        <div className="max-w-4xl mx-auto text-center">
          <h3 className="text-2xl font-bold text-white mb-4">LED MASK COLLECTION</h3>
          <p className="text-white/70 mb-6">Futuristic LED masks with stunning light effects</p>
          <div className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 inline-block">
