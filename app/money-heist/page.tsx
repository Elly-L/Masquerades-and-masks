...
const moneyHeistMasks: MoneyHeistMask[] = [
  {
    id: "mh-01",
    name: "Salvador Dali Mask",
    code: "MH-01",
    price: 500,
    image: "/images/money-heist/salvador-dali-mask.jpg",
    description: "Famous Salvador Dali mask from Money Heist series",
  },
]

export default function MoneyHeistPage() {
  ...
  const orderViaWhatsApp = (mask: MoneyHeistMask) => {
  ...
  const orderSelectedMasks = () => {
    ...
    const selectedMasksList = moneyHeistMasks.filter((mask) => selectedMasks.has(mask.id))
    ...
    const message = `Hi! I'd like to order the following Money Heist masks:\n\n${itemsList}\n\nTotal: KSH ${totalPrice}\n\nPlease confirm availability and delivery details.`
    const encodedMessage = encodeURIComponent(message)
    window.open(`https://wa.me/${whatsappNumber}?text=${encodedMessage}`, "_blank")
  }
  ...
  const totalPrice = moneyHeistMasks
    .filter((mask) => selectedMasks.has(mask.id))
    .reduce((sum, mask) => sum + mask.price, 0)

  return (
    <div className="min-h-screen bg-gradient-to-br from-red-900 via-yellow-900 to-orange-900 relative overflow-hidden">
      ...
      {showGuide && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <Button
              className="w-full bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white font-medium py-3 rounded-xl shadow-lg"
              onClick={() => setShowGuide(false)}
            >
      ...
      <div className="absolute inset-0 opacity-20">
        ...
        <div className="absolute bottom-20 left-40 w-32 h-32 md:w-72 md:h-72 bg-orange-500 rounded-full mix-blend-multiply filter blur-xl animate-pulse delay-2000"></div>
      </div>
      ...
      <header className="relative z-10 p-4 md:p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center">
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-white bg-gradient-to-r from-red-400 via-yellow-500 to-orange-600 bg-clip-text text-transparent mb-2">
              MONEY HEIST
            </h1>
            <p className="text-white/80 text-sm md:text-lg">Popular Collection</p>
          </div>
        </div>
      </header>
      ...
      {selectedCount > 0 && (
        <div className="fixed top-16 left-0 right-0 z-40 px-4">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto">
            <div className="backdrop-blur-md bg-gradient-to-r from-red-600/90 to-orange-600/90 rounded-2xl p-4 border border-white/20 shadow-xl">
          ...
          <div className="text-center mb-8">
            <div className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 inline-block">
              <div className="grid grid-cols-3 gap-6 text-center">
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-red-400">{moneyHeistMasks.length}</div>
                  <div className="text-white/70 text-xs md:text-sm">Popular Designs</div>
                </div>
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-yellow-400">KSH 500</div>
                  <div className="text-white/70 text-xs md:text-sm">Each Mask</div>
                </div>
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-orange-400">Famous</div>
      ...
      <section className="relative z-10 py-6 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {moneyHeistMasks.map((mask, index) => {
                ...
                >
                  ...
                  >
                    <div className="relative">
                      <div className="aspect-square bg-gradient-to-br from-red-500/20 to-yellow-500/20 flex items-center justify-center relative overflow-hidden">
                        <img
      ...
      <footer className="relative z-10 py-12 px-4 border-t border-white/20 mt-20">
        <div className="max-w-4xl mx-auto text-center">
          <h3 className="text-2xl font-bold text-white mb-4">MONEY HEIST COLLECTION</h3>
          <p className="text-white/70 mb-6">Popular masks from famous series</p>
          <div className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 inline-block">
